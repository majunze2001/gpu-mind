...
echo 'printk' | sudo tee -a set_ftrace_notrace
echo 'vprintk_func' | sudo tee -a set_ftrace_notrace
echo 'vprintk_default' | sudo tee -a set_ftrace_notrace
...
start=$(grep -n 'uvm' trace | head -n 1 | cut -d: -f1)
